...
bool    Functions3000prot::ReadAndCheckReceivedAnswer(int _timeoutMs, unsigned char& _pReceivedAnswer, int& _pAnswerLength)
{
    ...
    while((std::chrono::steady_clock::now() - starttime) < std::chrono::milliseconds(_timeoutMs))
    {
    ...
    if (availableBytes >= 5)
    {
        ...
        if (bytesToRead > 0)
        {
            unsigned char bytesBuffer2[64] = {0};
            int readBytesNum2 = 0;
            ...
            if (bytesToRead >= availableBytes)
            {
                ...
                readBytesNum2 = m_pConnection->ReadBytesFromPort(*bytesBuffer2, bytesToRead);
            }
            memcpy((&bytesBuffer + readBytesNum), bytesBuffer2, readBytesNum2);
