...
int main()
{
    //	C++11 required for std::chrono::steady_clock
	//	For LINUX: If "OpenCommunicationPort" fails, check if current user is part of group "dialout" (to add run: "sudo gpasswd --add [username] dialout" and restart)
    ...
    if (input == '1')
    {
        ...
        {
            //For LINUX
            std::cout << "Trying to open port... /dev/ttyUSB0" << std::endl;
            portIsOpen = rfidFunctions.OpenCommunicationPort("/dev/ttyUSB0");
            //For macOS
            //std::cout << "Trying to open port... /dev/tty.usbserial-00000001" << std::endl;
            //portIsOpen = rfidFunctions.OpenCommunicationPort("/dev/tty.usbserial-00000001");
